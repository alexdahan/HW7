...
UPDATE actor
SET Actor_Name = CONCAT(UPPER(actor.first_name), " " , UPPER(actor.last_name) );
...
UPDATE actor
SET Actor_Name = 'HARPO WILLIAMS', first_name= 'HARPO', last_name = 'WILLIAMS'
WHERE Actor_Name = 'GROUCHO WILLIAMS';

#4d. Perhaps we were too hasty in changing GROUCHO to HARPO. It turns out that GROUCHO was the correct name after all! 
#In a single query, if the first name of the actor is currently HARPO, change it to GROUCHO.
UPDATE actor
SET Actor_Name = 'GROUCHO WILLIAMS', first_name= 'GROUCHO', last_name = 'WILLIAMS'
...
SELECT staff.staff_id, staff.first_name, staff.last_name, sum(payment.amount) AS Total_Rung_Up 
FROM payment 
...
SELECT film.film_id, count(film_actor.actor_id) AS Number_of_Actors 
FROM film 
...
SELECT film.title FROM film WHERE title LIKE 'Q%' AND language_id = 1 OR title LIKE 'K%' AND language_id = 1;
...
SELECT film.title, count(film.film_id) AS "Rental_Counts"
FROM film 
...
CREATE OR REPLACE VIEW top_five_g AS
SELECT name, 'Gross Revenue'
FROM top_five_genres;

#8b. How would you display the view that you created in 8a?
SELECT * FROM top_five_g;

#8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW top_five_g;
